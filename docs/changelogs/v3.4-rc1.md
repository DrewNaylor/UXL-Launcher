...
- Any time a setting requires an application restart (for now, only turning on or off the theme engine), a messagebox will pop up asking the user if they want to restart now.
  - If this message is declined, it won't pop up again during that session even if the theme engine is turned on or off multiple times in that session. ([issue #177](https://github.com/DrewNaylor/UXL-Launcher/issues/177))
