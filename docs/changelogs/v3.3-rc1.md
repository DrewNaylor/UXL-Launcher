...
- Trying to load a custom theme that has something like "1.03..34" in the <UseThemeEngineVersion> node will crash UXL Launcher with an InvalidCastException when opening the Options window, choosing it with the "Browse for a custom theme" dialog, or when switching to using a custom theme and it's in the custom theme path textbox. ([issue #173](https://github.com/DrewNaylor/guinget/issues/173))
...
- "TenDark" theme button highlight colors are a bit hard to see. ([issue #140](https://github.com/DrewNaylor/guinget/issues/140))
- Fixed some typos in comments.

## Changed

- If the theme file specifies that it uses a newer version of the theme engine, a warning will be appended to the end of the theme details. ([issue #166](https://github.com/DrewNaylor/guinget/issues/166))
