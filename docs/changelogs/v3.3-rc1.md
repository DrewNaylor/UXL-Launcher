...
PortableThemeEngine/TE2.x is basically the UXL Launcher Theme Engine/TE1.x split into its own library so that other Windows Forms applications can use it, and it's licensed under the Apache License 2.0. The first version of it is version 2.0 as it's meant to be the successor to the built-in theme engine. Eventually it'll replace the built-in version as well, likely by the time UXL Launcher 4.0 is out.
